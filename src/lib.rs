...
impl Store {
    ...
    pub fn persist_network(txn: &WriteTransaction, network: &Network, wallet_name: &str) -> Result<(), BdkRedbError> {
        let mut table = txn.open_table(NETWORK)?;
    ...
    pub fn read_network(
        ...
        let table = txn.open_table(NETWORK)?;
